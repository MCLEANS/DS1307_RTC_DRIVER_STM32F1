...
void DS1307::set_time(RTC_Time rtc_time){
	...
	write_bytes(RTC_ADDRESS,0x00,set_buffer,BUFFER_SIZE);
}

RTC_Time DS1307::get_time(){
	RTC_Time rtc_time;

	write_memp(RTC_ADDRESS,0x00);
	read_bytes(RTC_ADDRESS,buffer,BUFFER_SIZE);
	rtc_time.second = BCD_to_decimal(buffer[0]);
